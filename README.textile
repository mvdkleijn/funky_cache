...
Vanilla Frog give out about 90 pages per second. These tests are done on Apache running on localhost to rule out network latency.
...
With Funky Cache only first request is passed to Frog. All following requests are served as cached static file  avoiding call to CMS altogether. Now you site can withstand Digg effect or being Slashdotted :)
